...
def permutation_test(t, null_dist):
    """This function estimates a p-value by comparing against a user-defined null
    distribution which should be generated by permutation testing of the data. The
    p-value is the fraction of the null distribution which is greater or smaller than
    the test statistics. However, the finite size of the generated null distribution
    results in some variability of this estimate. A correction is applied by estimating
    the fluctuation size with a binomial distribution. The expected variance in the
    number :math:`M` (number of data points from the null distribution exceeding the
    test statistics) with probability :math:`P` is described by the variance of the
    binomial distribution :math:`\sigma^2=NP(1-P)`. Here, a conservative upper bound of
    :math:`3\sigma` is set to consider the finite sampling size in the p-value
    estimation. The idea is taken from [1]_.
    ...
    p : float
        Estimated p-value.
    pcorr : float
        Corrected p-value with consideration of the finite sampling size.
    """
    N = float(len(null_dist))
    M = np.sum(np.abs(null_dist) > abs(t))
    P = M / N
